...
>
    <div class="section-container">
            ...
            <div class="hidden md:flex items-center space-x-8">
                {#each navItems as item}
                    ...
                    >
                        {item.label}
                        <span
                            class="absolute -bottom-1 left-0 w-0 h-0.5 bg-gradient-to-r from-primary-400 to-secondary-400 transition-all duration-300 group-hover:w-full"
        ...
        {#if isMobileMenuOpen}
            <div
                class="md:hidden absolute top-full left-0 right-0 backdrop-blur-lg border-b border-primary-500"
                ...
                <div class="px-4 py-6 space-y-4">
                    {#each navItems as item}
                        <a
                            ...
                            class:text-primary-300={$page.url.pathname ===
                                item.href}
                            class:neon-glow-sm={$page.url.pathname ===
                                item.href}
                        >
                            {item.label}
                        </a>
